...
> **Local Validation**: Before submitting, ensure your workspace passes all integrity checks using the project’s automation suite:
> ```bash
...
Every PR must satisfy the following technical and architectural invariants:

- **Automated Checks**:
  - Pass architectural integrity validation: `./scripts/workspace-check.sh`
  - Pass security and supply-chain auditing: `./scripts/security-audit.sh`
  - Pass full workspace build: `cargo build --workspace --all-targets`
- **Quality Assurance**:
  - Pass all tests: `cargo test --workspace --all-targets`
  - Pass linting: `cargo clippy -- -D warnings`
  - Pass formatting: `cargo fmt --all -- --check`
- **Documentation**: Include updated documentation when applicable.
- **Compliance**: Adhere strictly to the naming conventions and terminology defined in the Glossary.
